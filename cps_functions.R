...
#'
#' @examples
preprocess_em<- function(expresion_matrix){
...
#'
#' @examples
do_CPS_coarse <- function(expression_matrix){
...
#'
#' @examples
do_CPS_fine <- function(expression_matrix) {
  load("modesl/glmnet_model_finegrained.rdata")
...
do_CPS <- function(expression_matrix, type= NA){
  ...
  if (ype == "coarse"){
    results<-do_CPS_coarse(expression_matrix)
    }else{
      results<-do_CPS_coarse(expression_matrix)
    }
}
